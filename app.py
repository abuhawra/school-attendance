...
# --- 🎨 إعدادات التنسيق (CSS & Header) مخفية هنا لترتيب الكود ---
STYLE_CSS = '''
    <style>
    ...
    .wa-all { background-color: #28a745; } .wa-absent { background-color: #dc3545; } .wa-late { background-color: #fd7e14; }
...
HEADER_HTML = '''
    <div style="background-color: #1a237e; padding: 40px 20px; text-align: center; color: white; border-bottom: 8px solid #ffd700; border-radius: 15px; box-shadow: 0 4px 15px rgba(0,0,0,0.3); margin-bottom: 30px;">
        ...
        <div style="margin: 25px 0;"></div> <div style="font-size: 22px; font-weight: 500; color: #cfd8dc; margin-bottom: 5px;">فكرة وبرمجة</div>
        <div style="font-size: 30px; font-weight: 700; color: #ffd700; margin-bottom: 25px;">أ. عارف أحمد الحداد</div>
        <div style="font-size: 22px; font-weight: 500; color: #cfd8dc; margin-bottom: 5px;">مدير المدرسة</div>
...
# --- 📝 قسم رصد المعلم ---
    ...
    if res_s.data:
        ...
        if sel_c != "---":
            ...
            if st.button("💾 حفظ بيانات الرصد"):
                supabase.table('attendance').delete().eq("committee", sel_c).eq("date", today).execute()
                supabase.table('attendance').insert(results).execute()
                st.success("تم حفظ البيانات!"); time.sleep(1); st.session_state.page = "home"; st.rerun()
...
# --- ⚙️ لوحة الإدارة ---
    ...
    with tab1:
        ...
        if res_att.data:
            ...
            if not df_report.empty:
                st.table(df_report[['committee', 'student_name', 'status', 'teacher_name']].rename(columns={'committee':'اللجنة','student_name':'الطالب'}))
            else: st.success("لا يوجد غياب اليوم.")

    # --- التعديل هنا في تاب حالة اللجان ---
    with tab2:
        ...
        if res_s.data:
            ...
            # جلب بيانات رصد اليوم لمعرفة اسم المعلم لكل لجنة
            today_date = str(datetime.now().date())
            att_today = supabase.table('attendance').select("committee, teacher_name").eq("date", today_date).execute()
            
            # إنشاء قاموس يربط رقم اللجنة باسم المعلم (تجنباً للتكرار)
    ...
    with tab3:
        if st.text_input("رمز إدارة البيانات:", type="password") == "4321":
            ...
            # أزرار النسخة الاحتياطية والاستيراد تظل كما هي في كودك الأصلي
