...
# --- دالة جلب وحفظ حالة النظام من Supabase ---
...
# --- 🎨 التنسيق والمظهر (تصحيح الألوان والخطوط) ---
...
st.markdown('''
    ...
    .closed-msg { color: #ffffff; font-weight: 800; font-size: 22px; background-color: #d32f2f; padding: 15px; border-radius: 12px; border: 3px solid #ffd700; text-align: center; margin-top: 15px; box-shadow: 0px 4px 10px rgba(0,0,0,0.3); }
...
if st.session_state.page == "home":
    ...
    with col_b:
        if system_is_open:
            ...
            st.markdown('<div class="closed-msg">⚠️ النظام مغلق الآن <br> يفتح الرصد في تمام الساعة 7:10 صباحاً</div>', unsafe_allow_html=True)
...
# --- 2. تسجيل دخول المعلم ---
    ...
    if st.button("دخول"):
        ...
        else: st.error("السجل المدني غير مسجل في قاعدة البيانات.")
...
# --- 4. صفحة الشكر ---
    ...
    st.snow()
    st.markdown('<div style="text-align:center; padding:50px;"><h1>✅ تم الحفظ بنجاح</h1><p>شكراً لتعاونكم</p></div>', unsafe_allow_html=True)
...
# --- 5. لوحة الإدارة ---
    ...
    with tab3:
        if st.text_input("رمز حماية البيانات:", type="password") == "4321":
            ...
            with col_on:
                if st.button("🔓 فتح الرصد", use_container_width=True):
                    set_system_status(True); st.rerun()
            with col_off:
                if st.button("🔒 إغلاق الرصد", use_container_width=True):
                    set_system_status(False); st.rerun()
            ...
            if res_std.data:
                ...
                c1, c2 = st.columns(2)
                with c1: st.download_button("📥 Excel الطلاب", df_s.to_csv(index=False).encode('utf-8-sig'), "students.csv", use_container_width=True)
                with c2:
                    buf = io.BytesIO()
                    with pd.ExcelWriter(buf, engine='openpyxl') as wr: df_s.to_excel(wr, index=False)
                    st.download_button("📊 تحميل Excel", buf.getvalue(), "students.xlsx", use_container_width=True)
