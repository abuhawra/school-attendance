...
# 1. إعدادات الاتصال بقاعدة البيانات (سوبابيز)
...
# 2. تحسينات الواجهة والتنسيق (CSS)
...
# ==========================================
    ...
    sel_c = st.selectbox("اختر اللجنة لرصد الحضور:", ["---"] + coms)
    if sel_c != "---":
...
# ==========================================
    ...
    if st.button("⬅️ عودة"): st.session_state.page = "home"; st.rerun()
    if st.text_input("رمز الدخول للمسؤول:", type="password") == "1234": st.session_state.page = "admin_panel"; st.rerun()

elif st.session_state.page == "admin_panel":
    if st.button("⬅️ تسجيل الخروج"): st.session_state.page = "home"; st.rerun()
    tab1, tab2, tab3 = st.tabs(["📊 تقارير الغياب", "🏘️ متابعة حالة اللجان", "💾 إدارة البيانات والنسخ الاحتياطي"])
    ...
    with tab1:
        ...
        if res.data:
            ...
            else: st.success("لا يوجد غياب مرصود")
    ...
    with tab3:
        st.subheader("💾 تحميل النسخة الاحتياطية للطلاب")
        ...
        if std_res.data:
            ...
            with col_b1:
                ...
                st.download_button("📥 تحميل بصيغة CSV", data=csv_data, file_name=f"students_{today_date}.csv", mime="text/csv")
            with col_b2:
                # تصدير Excel (XLSX)
                output = io.BytesIO()
                ...
                st.download_button("📥 تحميل بصيغة Excel (XLSX)", data=excel_data, file_name=f"students_{today_date}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            st.divider()
            st.info(f"إجمالي عدد الطلاب المسجلين: {len(df_backup)}")
