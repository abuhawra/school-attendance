...
# --- 1. واجهة المعلم ---
if page == "🔑 دخول المعلم":
    # التحقق هل الصفحة مفتوحة أم مغلقة
    is_open = check_system_status()
    
    if not is_open:
        st.error("🚫 نظام رصد الغياب مغلق حالياً من قبل الإدارة.")
        st.info("يرجى المحاولة في وقت لاحق أو مراجعة المسؤول.")
    else:
...
# --- 2. لوحة الإدارة ---
    ...
    if password == "1234":
        # --- قسم التحكم في فتح وإغلاق النظام ---
        st.subheader("⚙️ إعدادات النظام")
        current_status = check_system_status()
        
        col_status, col_btn = st.columns([2, 1])
        with col_status:
            status_text = "🟢 النظام مفتوح حالياً" if current_status else "🔴 النظام مغلق حالياً"
        ...
        if att_res.data:
            df = pd.DataFrame(att_res.data).rename(columns={'student_name':'الاسم','committee':'اللجنة','status':'الحالة'})
            tab1, tab2 = st.tabs(["⚠️ كشف الغياب", "🚩 حالة اللجان"])
            with tab1: st.table(df[df['الحالة'].isin(['غائب','متأخر'])])
            with tab2: st.write("✅ اللجان المكتملة:", list(df['اللجنة'].unique()))
        else:
            st.warning("لا توجد بيانات لهذا اليوم.")
    else:
