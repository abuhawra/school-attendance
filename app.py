    ...
    if sel_c != "---":
        ...
        for s in students.data:
            ...
            stat = st.radio(f"👤 {s['student_name']}", ["حاضر", "غائب", "متأخر"], 
                            index=["حاضر", "غائب", "متأخر"].index(p_stat), key=f"radio_{s['id']}", horizontal=True)
...
# ==========================================
# ⚙️ 3. لوحة الإدارة والتقرير الموحد (النسخة المصححة)
# ==========================================
    ...
    tab1, tab2 = st.tabs(["📊 تقرير الغياب والواتساب", "🏘️ متابعة اللجان"])
    ...
    with tab1:
        ...
        if res.data:
            ...
            if not df_abs.empty:
                ...
                # 3. عرض الجدول بعناوين عربية صحيحة
                ...
                # 5. زر الواتساب الموحد
                st.markdown(f'<a href="https://wa.me/?text={msg}" target="_blank" class="whatsapp-btn">إرسال تقرير الغياب الموحد (واتساب) 📲</a>', unsafe_allow_html=True)
            else:
                st.success("✅ لا توجد حالات غياب أو تأخر في هذا التاريخ.")
        else:
            st.warning("لا توجد بيانات مرصودة لهذا التاريخ.")
