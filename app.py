...
# --- 3. لوحة الإدارة (إصلاح إدارة البيانات والنسخ الاحتياطي) ---
    ...
    with tab2: # حالة اللجان (سجل المعلمين)
        ...
        with c1:
            ...
            for c in all_c:
                if c in done_dict: 
                    st.markdown(f"📍 لجنة {c}: <span class='teacher-log-span'>{done_dict[c].replace(' | ', ' ⬅️ ')}</span>", unsafe_allow_html=True)
        with c2:
            st.error("❌ لم ترصد")
            for c in all_c:
                if c not in done_dict: st.write(f"⚠️ لجنة {c}")

    with tab3: # إدارة البيانات (هنا الإصلاح الأساسي)
        if st.text_input("رمز حماية البيانات:", type="password") == "4321":
            st.subheader("👨‍🎓 الطلاب و 👨‍🏫 المعلمين")
            
            # قسم المعلمين
            st.info("إدارة سجل المعلمين")
            res_t = supabase.table('teachers').select("*").execute()
            if res_t.data:
                df_t = pd.DataFrame(res_t.data)
                col1, col2 = st.columns(2)
                with col1:
                    st.download_button("📥 تحميل قائمة المعلمين (CSV)", df_t.to_csv(index=False).encode('utf-8-sig'), "teachers.csv", use_container_width=True)
                with col2:
                    up_t = st.file_uploader("رفع قائمة معلمين جديدة:", key="up_t")
                    if up_t and st.button("🚀 تحديث المعلمين"):
            ...
            # قسم الطلاب (النسخ الاحتياطي)
            st.info("إدارة سجل الطلاب")
            ...
            if res_s.data:
                ...
                with sc1:
                    ...
                    st.download_button("📊 تحميل نسخة الطلاب (Excel)", buf.getvalue(), "students.xlsx", use_container_width=True)
                with sc2:
                    up_s = st.file_uploader("رفع قائمة طلاب جديدة:", key="up_s")
