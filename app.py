...
st.markdown("""
    ...
    .status-card { padding: 10px; border-radius: 8px; margin: 5px 0; border: 1px solid #ddd; background-color: #ffffff; }
    </style>
...
# --- 📝 2. نظام رصد المعلمين ---
    ...
    sel_c = st.selectbox("اختر اللجنة لرصدها الآن:", ["---"] + coms)
    if sel_c != "---":
...
# --- ⚙️ 3. لوحة الإدارة (تقارير + متابعة + بيانات) ---
    ...
    # استعادة الشاشات الثلاث (التقارير، المتابعة، الإدارة)
    tab1, tab2, tab3 = st.tabs(["📊 تقارير الغياب الموحدة", "🏘️ متابعة حالة اللجان", "💾 إدارة وبيانات الطلاب"])
    
    today_date = str(datetime.now().date())

    with tab1:
        d = st.date_input("اختر تاريخ التقرير المطلوب", datetime.now())
        ...
        if res.data:
            ...
            if not df_abs.empty:
                ...
                # جلب الشعبة بشكل آمن (حل مشكلة APIError)
                classes = []
                ...
                msg = f"🗓️ *تقرير مدرسة القطيف*%0A📅 *التاريخ:* {d}%0A---------------------------------------%0A"
            ...
            else: st.success("لا توجد حالات غياب مسجلة في هذا اليوم")

    with tab2:
        st.subheader(f"🏘️ متابعة حالة رصد اللجان ليوم: {today_date}")
        ...
        with c1:
            ...
            for c in total_coms:
                if c in done_coms: st.write(f"📍 لجنة **{c}** (رصدها: {done_coms[c]})")
        with c2:
            st.error("❌ لجان لم ترصد بعد")
            for c in total_coms:
                if c not in done_coms: st.write(f"⚠️ لجنة رقم: **{c}**")

    with tab3:
        st.subheader("💾 إدارة البيانات والنسخ الاحتياطي")
        if st.button("📥 تصدير قائمة الطلاب (CSV)"):
            std_res = supabase.table('students').select("*").execute()
            if std_res.data:
                csv = pd.DataFrame(std_res.data).to_csv(index=False).encode('utf-8-sig')
                st.download_button("تحميل الملف الآن", data=csv, file_name=f"students_backup_{today_date}.csv", mime="text/csv")
        
        st.divider()
        st.info(f"إجمالي الطلاب في قاعدة البيانات: {len(supabase.table('students').select('id', count='exact').execute().data)}")
